...
$env:VCPKG_DEFAULT_TRIPLET="$ARCH-windows"
...
.\vcpkg.exe --triplet=$ARCH-windows install
.\vcpkg.exe --triplet=$ARCH-windows integrate install
...
cmake -A "$CMAKE_ARCH" -DVERBOSE_CMAKE=1 -DGETOPT_LIB_DIR="$VCPKG_PATH/packages/getopt-win32_$ARCH-windows/lib" -DGETOPT_INCLUDE_DIR="$VCPKG_PATH/packages/getopt-win32_$ARCH-windows/include" -DOPENSSL_PKG_PATH="$VCPKG_PATH/packages/openssl_$ARCH-windows/lib/pkgconfig" -DCMAKE_INSTALL_PREFIX="$RELEASE_DIR" ..
